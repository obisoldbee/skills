...
| `type_class` | Operational extension class; not a truth or reliability inference |
...
File extensions select a fail-closed operational allowlist only. They do not prove media type,
authorship, reliability, or factual content. Unsupported files must be converted or admitted by a
future reviewed package version; never silently relabel them.
...
search attachment caches. Missing, linked, directory, unreadable, or unsupported inputs return
`not_preserved` and create no record.
...
- unsupported file extensions or nonportable names;
...
different root, provider, path, type, version, or classification.
