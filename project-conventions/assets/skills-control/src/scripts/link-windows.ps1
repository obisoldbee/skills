...
# Scan or explicitly create collection Skill junctions on Windows.
...
$CollectionRoot = (Resolve-Path (Join-Path $ScriptDir '..\..\..')).Path
$ExportsFile = Join-Path $ScriptDir '..\config\skill-exports.tsv'
$TargetsFile = Join-Path $ScriptDir '..\config\agent-paths.tsv'

if (-not (Test-Path -LiteralPath $ExportsFile -PathType Leaf) -or
    -not (Test-Path -LiteralPath $TargetsFile -PathType Leaf)) {
    throw "config-missing: $ScriptDir\..\config"
}
...
Write-Host "mode=$Mode collection=$CollectionRoot"
...
foreach ($TargetEntry in $Targets) {
    ...
    if ($Apply -and
        ($TargetPath.Equals($CollectionRoot, [StringComparison]::OrdinalIgnoreCase) -or
         $TargetPath.StartsWith($CollectionRoot + [IO.Path]::DirectorySeparatorChar, [StringComparison]::OrdinalIgnoreCase))) {
        throw "target-inside-project-collection: $TargetPath"
    }
    ...
    foreach ($Export in $Exports) {
        ...
        $SourceCandidate = Join-Path $CollectionRoot $SourceRelative
        ...
        if (-not $Source.StartsWith($CollectionRoot + [IO.Path]::DirectorySeparatorChar, [StringComparison]::OrdinalIgnoreCase)) {
            Write-Host "source-outside-collection $SkillName $Source"
            $Conflicts++
            continue
        }
