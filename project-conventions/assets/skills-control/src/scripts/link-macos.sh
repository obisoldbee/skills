#!/usr/bin/env bash
# Scan or explicitly create collection Skill symlinks on macOS/Linux.
...
exports_file="$script_dir/../config/skill-exports.tsv"
targets_file="$script_dir/../config/agent-paths.tsv"

if [ ! -f "$exports_file" ] || [ ! -f "$targets_file" ]; then
  echo "error config-missing: $script_dir/../config" >&2
  exit 2
fi
...
echo "mode=$mode collection=$collection_root"
...
for target_index in "${!target_paths[@]}"; do
  ...
  if [ "$apply" -eq 1 ]; then
    case "$target_resolved" in
      "$collection_root"|"$collection_root"/*)
        echo "error target-inside-project-collection: $target_resolved" >&2
        exit 2
        ;;
    esac
  fi
  ...
  while IFS="$(printf '\t')" read -r skill_name source_rel consumers; do
    ...
    source_candidate="$collection_root/$source_rel"
    ...
    case "$source" in
      "$collection_root"/*) ;;
      *)
        echo "source-outside-collection $skill_name $source"
        conflicts=$((conflicts + 1))
        continue
        ;;
    esac
