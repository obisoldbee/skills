...
2. Named full initialization: clone the distribution repository directly as the user-named `project-conventions` Project Root's `src/` Repository Root, require the package at `src/project-conventions/SKILL.md`, initialize and read back the named target's three-file routing overlay, atomically migrate only the named sources, and propose one final-path Skill link. Reject or safely flatten the obsolete redundant `src/skills/` layout. A fresh task is not required merely to read an explicitly named local Skill file.
