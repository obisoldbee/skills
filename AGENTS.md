...
For lifecycle 2, use the current Project Root's mapped `src/<repository-name>/` checkout unless the user supplied another exact child path. Read `project-conventions/references/lifecycle-workflows.md` and `project-conventions/references/migration-guide.md`. If that checkout has a clean attached local default branch that is ahead/diverged, only an explicit full-chain authorization may preserve it by renaming it to `<branch>-preserved-<short-head>` and creating a fresh tracked default branch from the fetched remote; verify both refs and never rebase, reset, delete, or auto-push the preserved branch. Update-only must still stop. Snapshot hidden files and Git state, reserve incoming target names, and use the exact source-to-destination map already authorized by the user. Do not ask the user to choose unrelated archive/keep/delete options when both ends of a move were explicit. Before moving the active workspace, switch to a safe parent; if the host locks it, request only the necessary reopen-at-parent handoff. Link only after migration and final-path validation.
