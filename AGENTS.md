...
2. Named full initialization: clone the distribution repository directly as the user-named `project-conventions` Project Root's `src/` Repository Root, require the package at `src/project-conventions/SKILL.md`, initialize and read back the named target's three-file routing overlay, and atomically migrate only the named sources. Move a named existing collection-control Project Root whole; if none exists, use `project-conventions/scripts/initialize_skills_control_project.py` after the member checkout reaches its final path and never handwrite a reduced README-and-config substitute. Propose one final-path Skill link only after initialization. Reject or safely flatten the obsolete redundant `src/skills/` layout. A fresh task is not required merely to read an explicitly named local Skill file.
...
For lifecycle 2, use `src/` itself as the Repository Root and `src/project-conventions/` as the managed package; never derive `src/skills/` from the remote repository name. Read `project-conventions/references/lifecycle-workflows.md` and `project-conventions/references/migration-guide.md`. If that checkout has a clean attached local default branch that is ahead/diverged, only an explicit full-chain authorization may preserve it by renaming it to `<branch>-preserved-<short-head>` and creating a fresh tracked default branch from the fetched remote; verify both refs and never rebase, reset, delete, or auto-push the preserved branch. Update-only must still stop. After bounded collision and Git checks, use the package initializer as the first target write and immediately read back `AGENTS.md`, `README.md`, and `MEMBERS.md`; do not recursively scan large repositories before this checkpoint. Snapshot hidden files and Git state before each move, reserve incoming target names, and use the exact source-to-destination map already authorized by the user. Do not ask the user to choose unrelated archive/keep/delete options when both ends of a move were explicit. Before moving the active workspace, switch to a safe parent; if the host locks it, request only the necessary reopen-at-parent handoff. Link only after migration and final-path validation.
...
- A named bootstrap placed the package at `src/project-conventions/SKILL.md`, not `src/skills/project-conventions/SKILL.md`, and target initialization produced a three-file readback before migration.
