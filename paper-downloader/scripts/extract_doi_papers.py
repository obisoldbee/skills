#!/usr/bin/env python3
"""Export DOI browser inputs from the canonical manifest without side effects."""
...
def extract_rows(manifest: dict) -> list[dict]:
    return [
        ...
        if doi_followup_required(row, DEFAULT_FOLLOWUP_STATUSES)
        if (item := doi_item(row)) is not None
    ]
