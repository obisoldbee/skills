...
- 治理：Codex 原生排除、浏览器路线的 `provider_execution_authority` / `visible_task_creation_authority` 分轴合同、当前任务浏览器优先与显式授权的 Luna→ego-browser 交接、MMX 外置且 music API eligibility-gated、其他 provider `observed_not_routed`、凭据不入包。
