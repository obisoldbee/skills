...
| 模式 | MMX Legacy | MMX H3 | Agnes 2.5 Flash（默认 Agnes） | Agnes 2.5（显式选择） |
|---|---|---|---|---|
| 文生视频 | 支持 | 支持 | `text` | `text` |
| 单图生视频 | 支持 | 支持 | 单首帧或单尾帧 | 单首帧或单尾帧 |
| 首尾帧 | 支持 | 支持 | `keyframe`；首尾至少一个 | 同 Flash |
| 单主体参考图 | S2V-01 | 参考图 | `reference` 图片 | 同 Flash |
| 多参考图 | 不支持 | 支持 | 最多 5 张 | 最多 8 张 |
| 参考视频 | 不支持 | 支持 | 不支持 | 最多 1 段 |
| 参考音频 | 不支持 | 支持，且需同时有参考图或视频 | 最多 3 段，可单独使用 | 最多 3 段，可单独使用 |
| 精确原视频编辑 | 不承诺 | 不承诺 | 不承诺 | 不承诺 |

Agnes 图片使用 `agnes-image-2.5-flash`。视频两个 2.5 模型共用 4–12 秒、三种互斥模式、`n=1` 和携带模型的任务查询；Flash 仅 720P。不能因输入或分辨率超出 Flash 限制自动切到标准版。完整参数与参考素材元数据限制见 [agnes-video.md](agnes-video.md)。
